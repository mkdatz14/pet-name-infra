...
This repository includes a manual GitHub Actions workflow at `.github/workflows/hcp-terraform-targeted-apply.yml` that queues runs for one or more HCP Terraform workspaces in the same environment branch. It is intended to sit alongside the existing VCS connection so you can trigger deployments such as `dev2` or `staging1,staging3` without applying the other workspaces in the same environment.
...
3. You enter one or more deployment numbers, separated by commas, and choose whether the run should auto-apply.
4. The workflow derives the environment from the branch name and resolves keys such as `dev2` or `staging3` from `HCP_TERRAFORM_WORKSPACE_MAP`.
5. It starts one job per requested deployment.
6. Each job checks that its target workspace does not already have a non-final run.
7. Each job queues a run through the HCP Terraform Runs API using the workspace's current VCS-backed configuration version.

If you run the workflow from a branch other than `dev`, `staging`, or `prod`, it fails immediately.

Because the workflow only creates runs for the deployment numbers you provide, it does not fan out to the other workspace deployments.
